...
@interface YMShareView ()

@property (nonatomic, strong) YMShareSheetView *shareSheetView; /**< 分享面板 */
...
@implementation YMShareView

+ (instancetype)shareViewWithShareItems:(NSArray *)shareArray
                          functionItems:(NSArray *)functionArray
{
    YMShareView *shareView = [[self alloc] initWithShareItems:shareArray functionItems:functionArray];
...
- (void)commonInit
{
    self.frame = CGRectMake(0, 0, YM_ScreenWidth, YM_ScreenHeight);
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(hide) name:YM_HideNotification object:nil];
}
...
{
    [UIView animateWithDuration:YM_AnimateDuration animations:^{
        self.dimBackgroundView.alpha = YM_DimBackgroundAlpha;
        
        CGRect frame = self.shareSheetView.frame;
        frame.origin.y = YM_ScreenHeight - self.shareSheetView.shareSheetHeight;
...
- (void)hideAnimationWithCompletion:(void (^)(BOOL finished))completion
{
    [UIView animateWithDuration:YM_AnimateDuration animations:^{
        ...
        CGRect frame = self.shareSheetView.frame;
        frame.origin.y = YM_ScreenHeight;
        self.shareSheetView.frame = frame;
    } completion:completion];
}
...
- (YMShareSheetView *)shareSheetView
{
    if (!_shareSheetView) {
        _shareSheetView = [[YMShareSheetView alloc] init];
        _shareSheetView.frame = CGRectMake(0, YM_ScreenHeight, YM_ScreenWidth, _shareSheetView.initialHeight);
...
- (UIView *)dimBackgroundView
{
    if (!_dimBackgroundView) {
        ...
        _dimBackgroundView.frame = CGRectMake(0, 0, YM_ScreenWidth, YM_ScreenHeight);
